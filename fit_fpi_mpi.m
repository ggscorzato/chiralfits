function [y, dy, chi2_P, dev, a_lat, z_estim, dz] = fit_fpi_mpi(search,myestim,parm)
% function for fitting m_pi and f_pi from lattice data (using Finite volume chpt formula).
% author: Luigi Scorzato
% last updated 6/12/2007 (used for fits of Nf=2 tm-see-tm-val data).
...
lec =  [0.4369  57.2738 0.6082   6.0821   2.9904   6.3355    -1   0   1.0  1.0 ];
range= [0.0025  10.0    0.2027   0.3041   5.0684   0.7096    1   1   0.1  0.1 ];
...
dm_pcac_a= abs(m_pcac_a) .*sqrt(((tab(:,indmpcacx+1)')./abs(tab(:,indmpcacx)')).^2+(dZA./ZA).^2);
f_pi=tab(:,indfpi)'/sqrt(2);
df_pi=tab(:,indfpi+1)'/sqrt(2);
if (assumemaxtwist==0)
 m_ov_mu=sqrt(abs(m_pcac_a).^2 + mu.^2)./mu;
 f_pi = m_ov_mu .* f_pi;
 df_pi = m_ov_mu .* df_pi;
end
m_pi2=m_pi.^2;
...
%y=z_estim;
%dy=dz;
% To determine also the Merit figure from the incomplete Gamma function use:
% Q=1- gammainc(chi2/2,dof/2);
%%%%%%%%%%%%%%%%%%%%%%% PLOTS
%%%% colors
ic=1;
